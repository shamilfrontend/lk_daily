...
export async function listHolidayTransfers(req: Request, res: Response): Promise<void> {
  ...
  const rows = await HolidayTransfer.find({ year }).sort({ fromDate: 1, toDate: 1 }).lean();
  res.json({
    year,
    items: rows.map((row) => ({
      id: row._id.toString(),
