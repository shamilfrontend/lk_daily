...
<template>
  <AppModal
    ...
    @close="onClose"
  >
    <form class="login-form" :aria-describedby="ariaDescribedBy" @submit.prevent="onSubmit">
      <p v-if="hint" id="login-modal-hint" class="login-form__hint muted">{{ hint }}</p>
      ...
      <p v-if="error" id="login-modal-error" class="error" role="alert" aria-live="polite">{{ error }}</p>

      <button class="btn btn--primary login-form__submit" type="submit" :disabled="loading">
        {{ loading ? 'Вход…' : 'Войти' }}
      </button>
    </form>
  </AppModal>
